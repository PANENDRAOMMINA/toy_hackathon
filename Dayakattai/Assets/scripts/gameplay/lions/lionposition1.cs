...
public class lionposition1 : MonoBehaviour
{
    public static lionposition1 instance;
    public Transform[] position;
    public GameObject[] circle=new GameObject[12];
    [SerializeField] private int p = 0, firstcount = 0, selectednumber = 0,prevoiusposition=0;
    public int i = 0,entrytosafezone=0,boxcount=0;
        ...
        if (!playerboolmove)
        {
            if (!GameManager.instance.powerup5forsquare)
            {
                if (steps == 1)
                {
                   ...
                   // positionsscript.instance.lionpiecetoposition(0);
                    i++;
                    playerboolmove = true;
