...
public class snakepositions2 : MonoBehaviour
{
    public static snakepositions2 instance;
    public Transform[] position = new Transform[79];
    public Vector2 defaultposition;

    public GameObject[] box = new GameObject[12];
    [SerializeField] private int p = 0, firstcount = 0, selectednumber = 0, entrytosafezone;
        ...
        else
        {
            if (entrytosafezone == 7 || entrytosafezone == 13 || entrytosafezone == 19 || entrytosafezone == 25 || entrytosafezone == 31 || entrytosafezone == 37 || entrytosafezone == 43 || entrytosafezone == 49 || entrytosafezone == 55 || entrytosafezone == 67)
            {
                ...
                if (canmove)
                {
                    ...
                    {
                      //  positionsscript.instance.removelion(previousnumber);
                        while (p < steps)
                        {
                            transform.position = position[i].transform.position;
                            b.transform.position = position[i].transform.position;
            ...
            else
            {
                if (entrytosafezone <= 80)
                {
                    //positionsscript.instance.removesnake(previousnumber);
                    while (p < steps)
                    {
                        transform.position = position[i].transform.position;
                        b.transform.position = position[i].transform.position;
