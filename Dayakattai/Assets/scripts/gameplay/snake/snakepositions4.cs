...
public class snakepositions4 : MonoBehaviour
{
    public static snakepositions4 instance;
    public Transform[] position=new Transform[80];
    public Vector2 deafultposition;
    public GameObject[] box = new GameObject[2];
    ...
    public int i = 0,boxcount=0,previousnumber=0;
        ...
        else
        {
            if (entrysafezone == 7 || entrysafezone == 13 || entrysafezone == 19 || entrysafezone == 25 || entrysafezone == 31 || entrysafezone == 37 || entrysafezone == 43 || entrysafezone == 49 || entrysafezone == 55 || entrysafezone == 67)
            {
                ...
                if (canmove)
                {
                    if (entrysafezone <= 80)
                    {
                      //  positionsscript.instance.removesnake(i);
                        while (p < steps)
                        {
                            transform.position = position[i].transform.position;
                            b.transform.position = position[i].transform.position;
            ...
            else
            {
                if (entrysafezone <= 80)
                {
                   // GetComponent<positionsscript>().removesnake(previousnumber);
                    while (p < steps)
                    {
                        transform.position = position[i].transform.position;
                        b.transform.position = position[i].transform.position;
                    ...
                    //GetComponent<positionsscript>().removesnake(i-1);
                    // previousnumber = i-1;
                }
                else
                {
